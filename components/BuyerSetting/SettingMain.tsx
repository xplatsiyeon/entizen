...
const SettingMain = ({
  ...
  const handleLogoutOnClickModalClick = async () => {
    const LOG_OUT_API = `https://ㅅㄷㄴㅅ-api.entizen.kr/api/members/logout`;
  ...
  const ModalLeftControl = async () => {
    const WITHDRAWAL_API = `https://ㅅㄷㄴㅅ-api.entizen.kr/api/members/withdrawal`;
    const accessToken = JSON.parse(localStorage.getItem('ACCESS_TOKEN')!);
    console.log('탈퇴');
    try {
      await axios({
  ...
  const authPassowrd = () => {
    ...
    if (checkPassword) {
      const LOGIN_API = 'https://ㅅㄷㄴㅅ-api.entizen.kr/api/members/login';
